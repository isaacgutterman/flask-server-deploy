...
{
    public TextAsset Transcripts; // CSV containing transcripts
    public TextAsset HouseCords; // CSV containing coordinates (x, y, z)
    ...
    {
        if (Transcripts == null || Transcripts.text == null || HouseCords == null || HouseCords.text == null)
        {
        ...
        var transcriptsData = ParseCSV(Transcripts.text, true);
        var houseCordsData = ParseCSV(HouseCords.text, true);

        // Ensure both datasets have the same number of rows
        int numRows = Mathf.Min(transcriptsData.Count, houseCordsData.Count);

        // Iterate through each row
        for (int i = 0; i < numRows; i++)
        {
            // Extract transcript from CSV 1 (assuming it's in the fourth column, index 3)
            if (transcriptsData[i].Length < 4)
            {
                Debug.LogWarning($"Not enough columns in row {i + 1} of CSV 1.");
                continue;
            }
            string transcript = transcriptsData[i][3];

            // Generate puzzle room name based on transcript
            yield return StartCoroutine(MakeName(transcript, i, transcriptsData, houseCordsData));
            yield return new WaitForSeconds(1f);
        }
    }
    private IEnumerator MakeName(string transcript, int rowIndex, List<string[]> transcriptsData, List<string[]> houseCordsData)
    {
        bool nameGenerated = false;
        string generatedName = "";
        ...
        if (!string.IsNullOrEmpty(generatedName))
        {
            string originalName = transcriptsData[rowIndex][0].Trim(); // Assuming transcript is in the fourth column
            nameMappings[originalName] = generatedName;
            CreateNameObject(generatedName, rowIndex, houseCordsData);
        }
        else
        {
    ...
    private void CreateNameObject(string nameContent, int rowIndex, List<string[]> houseCordsData)
    {
        // Display puzzle room name at the corresponding coordinates from CSV 2
        if (houseCordsData[rowIndex].Length >= 4)
        {
            if (float.TryParse(houseCordsData[rowIndex][0], out float x) &&
                float.TryParse(houseCordsData[rowIndex][1], out float z))
            {
